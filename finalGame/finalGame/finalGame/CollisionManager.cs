    ...
    public class CollisionManager : GameComponent
    {
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            foreach (Alien alien in alienList)
            {
                ...
                if(ship.BulletList.Count != 0) // doesn't run if there aren't bullets, memory saver
                {
                    Console.WriteLine($"Ship bullet count: {ship.BulletList.Count}");
                    foreach (Bullet bullet in ship.BulletList)
                    {
                        Rectangle bulletRect = bullet.getBound(); // checks the bullet boundary
            ...
            // Manage aliens colliding with ship
            foreach (Alien alien in alienList)
            {
                Rectangle alienRect = alien.getBound();
                Rectangle shipRect = ship.getBound();
                // check if any alien intersects with the ship (and only once so it doesn't spam the explosion)
                if(alienRect.Intersects(shipRect) && ship.Enabled == true)
                {
                    // disable ship if hit (action scene checks for disabled ships)
                    ship.Enabled = false;
                    ship.Visible = false;
                    // add explosion
                    explosion = new Explosion(game, spriteBatch, explosionTex, ship.Position);
